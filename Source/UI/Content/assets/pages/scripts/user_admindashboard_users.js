...
zw.page = {
    strings: {
        ...
        selectedAdvertismentsRemoved: "کاربران انتخاب شده حذف شدند",
    },
    ...
    initialize: function () {
        ...
        $("#remove-users").click(this.removeUsers);
    },
                ...
                { data: 'LastLoginDateString', title: zw.page.strings.lastLoginDateString },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return ('<div><a  target="_blank" href="{0}" class="btn btn-sm btn-outline grey-salsa">' +
                         ...
                         '</a>').replace('{0}', '/profile/' + full.Id).replace('{1}', zw.strings.view);// +
    ...
    removeUsers: function (e) {
        ...
        $.ajax("/user/admindashboard/removeUsers", {
            type: "POST",
            data: { ids: selectedRowsIds },
            success: function (response) {
                $('.contents-grid').DataTable().ajax.reload();
